...
namespace ns3
{
    ...
    main(int argc, char *argv[])
    {
        ...
        // consumerHelper.SetAttribute("Tos", UintegerValue(5));
        consumerHelper.SetAttribute("DelayStart", UintegerValue(80000));  // 延迟30秒启动
        consumerHelper.Install("C2");

        // C3
        consumerHelper.SetPrefix("/C");
        // consumerHelper.SetAttribute("Tos", UintegerValue(5));
        consumerHelper.SetAttribute("DelayStart", UintegerValue(160000));  // 延迟60秒启动
        consumerHelper.SetAttribute("DelayGreedy", IntegerValue(240000));   // 150秒开始贪婪
        ...
        ndn::L3RateTracer::InstallAll("dsccp3-1-throughput-320.txt", Seconds(0.5));
        ndn::AppDelayTracer::InstallAll("dsccp3-1-delay-320.txt");
        L2RateTracer::InstallAll("dsccp3-1-drop-320.txt", Seconds(0.5));
        Simulator::Run();
        Simulator::Destroy();
